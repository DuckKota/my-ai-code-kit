...
Defines how `setup install` builds the codebase-memory graph on first init: a synchronous, idempotent, best-effort index shared across both agents, so a freshly initialized project has a usable graph.
